...
function getReqHeaderValue(reqHeaders, headerKey) {
  if (typeof reqHeaders === 'function') {
    return reqHeaders(headerKey);
  } else if (reqHeaders && typeof reqHeaders === 'object') {
    return reqHeaders[headerKey];
  } else {
    throw new Error("req.header is not a function or an object");
  }
}

function getRequestUrl(req) {
  if (getReqHeaderValue(req.header, 'X-Requested-URI')) {
    return getReqHeaderValue(req.header, 'X-Requested-URI');
  } else {
    return req.protocol + '://' + getReqHeaderValue(req.header, 'Host') + req.url;
  }
}
...
function middleware(req, res, next) {
  ...
  let certificate = getReqHeaderValue(req.header, HEADER_CERTIFICATE);
  let providerId = getReqHeaderValue(req.header, HEADER_PROVIDER_ID);
  let signature = getReqHeaderValue(req.header, HEADER_SIGNATURE);
